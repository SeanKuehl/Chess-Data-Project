...
class Analyze():
    ...
    def WriteNotation(self, side, fmvn, sanMove, bookMove, posScore,
        ...
        with open(self.outfn, 'a+') as f:
            f.write(str(fmvn)+","+str(posScore)+","+str(engScore)+"\n")
    ...
    def SavegameHeaders(self):
        ...
        with open(self.outfn, encoding='ISO-8859-1') as h:
            while True:
                ...
                header.update({'Move Number': str(self.NUMBER_OF_LAST_MOVE)})
                header.update({'Move Evaluations': str(self.MOVE_EVAL_LIST)})
    ...
    def AnnotatePgn(self):
        ...
        with open(self.infn, encoding='ISO-8859-1') as pgnHandle:
            while True:
                ...
                tmpHeader.update({'Move Number': str(self.NUMBER_OF_LAST_MOVE)})
                tmpHeader.update({'Move Evaluations': str(self.MOVE_EVAL_LIST)})
                ...
                valid_headers_list = ['Date', 'Round', 'Result', 'BlackElo', 'BlackTitle', 'ECO',
                                      'Opening', 'Variation', 'WhiteElo', 'WhiteTitle']

                with open(self.outfn, 'a') as f:
                    for key, value in game.headers.items():
                        if key in valid_headers_list:
                            f.write('%s,%s\n' %(key, value))

                    #f.write('%s,%s\n' %('Move Number', str(self.NUMBER_OF_LAST_MOVE)))
                   # f.write('%s,%s\n' %('Move Evaluations', str(self.MOVE_EVAL_LIST)))
                ...
                while gameNode.variations:
                    ...
                    if not isGameOver:
                        if self.evalType == 'static':
                            if chess.Board(nextFen).is_check():
                                posScore = None
                            else:
